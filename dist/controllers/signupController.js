...
const createUserFunction = asyncHandler(async function (req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        if (errorMessages.includes("Username is required") ||
        ...
        else if (errorMessages.includes("Password must be at least 8 characters long") ||
            ...
            throw new AppError("Resource trying to create already exists", 409, "duplicate_resource", errors.array());
        }
        throw new AppError("Internal server error", 500, "internal_server_error", errors.array());
    }
    else {
        ...
        sendSuccess(res, { user: createdUser, token, refreshToken }, 201, "User created successfully");
    }
});
